...
- Dans `Partager`, l'entraîneur connecté peut voir les liens publics d'équipe qui appartiennent à son compte, même si le navigateur courant n'a plus la référence locale active. Il peut copier ou retirer ces liens depuis l'app. Retirer un lien d'équipe supprime seulement `publicTeams/{teamPublicId}`; les liens publics de match restent gérés séparément.
