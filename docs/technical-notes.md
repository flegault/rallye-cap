...
- `#partager`: `Partages`, exports et liens publics de l'équipe courante, accessible par le menu global.
...
Le menu du haut est un menu global unique qui regroupe `Accueil`, `Partages`, `Connexion` et `Réinitialiser`. Le sélecteur d'équipe active et l'action `Nouvelle équipe` vivent dans `Accueil`. `Matchs` reste une page séparée avec la route officielle `#matchs`, mais elle est accessible depuis l'accueil plutôt que depuis le menu. `Spectateur` n'est plus dans le menu et s'ouvre depuis une ligne admissible de `Matchs`. Les étapes `Match`, `Joueurs` et `Alignement` restent visibles dans le contenu via le workflow numéroté, pas dans le header. La création d'équipe exemple apparaît seulement dans `Accueil` quand aucune équipe n'existe.

Pendant le développement, les routes désuètes ne sont pas maintenues. `#equipe` et `#mesmatchs` ne sont pas des alias: comme toute route inconnue, elles retournent à `#accueil`.
...
- navigation cible `Accueil`, `Matchs`, `Match`, `Joueurs`, `Alignement`, `Partages`, `Spectateur`;
