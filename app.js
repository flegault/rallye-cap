...
let state=loadState();
let cloud={module:null,user:null,enabled:null,status:'local',matchId:null,publicId:null,publicPassword:'',teamPublicId:null,teamPublicPassword:'',unsubAuth:null,unsubMatch:null,unsubPublic:null,unsubPublicTeam:null,matchListenToken:0,lastRemoteMs:0,lastLocalMs:0,lastPayloadJson:'',saving:false,pendingSave:false,clientId:cloudClientId,publicView:null,publicIdRoute:null,publicTeamView:null,publicTeamRoute:null,publicFavoriteContext:null,publicFavoriteKeys:[],publicCurrentIndex:null,publicUpdateAvailable:false,matches:[],matchesLoading:false,matchesLoaded:false,publicTeams:[],publicTeamsLoading:false,publicTeamsLoaded:false};
let cloudSaveTimer=null,publicTeamSaveTimer=null,suppressCloudSave=false;
let analysis=null, problems=new Map(), dragRow=null, dragCell=null, selectedRow=null, selectedCol=null, selectedHalf=null, highlightPos=null, statSort={key:'order',dir:1}, matchTableSort={active:{key:'date',dir:-1},archived:{key:'date',dir:-1}}, matchIndex=0, spectatorTouched=false, touchStartX=0, optimizeDirty=true, lineupAutoDirty=false, addPlayersExpanded=false, selectedArchiveId=null, lineupMode='preparer';
function normalizeRoute(r){return['accueil','matchs','match','joueurs','alignement','spectateur'].includes(r)?r:'accueil'}
function routeTitle(r){let names={accueil:'Accueil',matchs:'Matchs',match:'Match',joueurs:'Joueurs',alignement:'Alignement',spectateur:'Spectateurs en direct'};return (names[r]||'Alignement')+' - Rallye-Cap'}
...
function renderCloudUi(){
  ...
  if(quick){quick.classList.toggle('hide',!quickVisible);quick.href=quickVisible?'#alignement':'#accueil';quick.textContent=m?.status==='completed'?'Match terminé':'Match en cours'}
  renderShareStep();
...
function renderPublicMatch(){let view=cloud.publicView,steps=publicSteps(view);if(!view||!steps.length){renderPublicError('Match non prêt','Alignement à venir.');return}let currentStep=publicCurrentStepIndex(view);if(cloud.publicCurrentIndex!==null&&currentStep!==cloud.publicCurrentIndex){let wasFollowing=!spectatorTouched||matchIndex===cloud.publicCurrentIndex;if(wasFollowing){matchIndex=currentStep;cloud.publicUpdateAvailable=false}else cloud.publicUpdateAvailable=true}if(!spectatorTouched)matchIndex=currentStep;cloud.publicCurrentIndex=currentStep;if(matchIndex>=steps.length)matchIndex=steps.length-1;if(matchIndex<0)matchIndex=0;let step=steps[matchIndex],notice=cloud.publicUpdateAvailable?'<div class="matchNotice">Nouvelle demi-manche disponible <button type="button" data-public-current>Voir</button></div>':'';if(step.kind==='program'){$('#matchCard').innerHTML=notice+renderPublicProgram(view)}else if(step.kind==='final'){$('#matchCard').innerHTML=notice+renderPublicFinal(view)}else{let ph=step,locked=!!ph.locked,currentShown=view.started&&matchIndex===currentStep,title=ph.type==='attaque'?'🏏 Frappeurs':'🧤 Défenseurs',body=ph.type==='attaque'?renderPublicBatters(ph.inning):renderPublicDefense(ph.inning);$('#matchCard').innerHTML=notice+'<div><span class="matchPill '+(locked?'done':currentShown?'current':'future')+'">'+esc(ph.label)+'</span><h2 class="matchTitle publicPhaseTitle">'+esc(title)+'</h2><div class="tiny matchMeta publicMetaLine">'+esc(publicMeta(view))+'</div></div><div class="matchList">'+body+'</div>'}let noticeBtn=$('[data-public-current]');if(noticeBtn)noticeBtn.onclick=()=>{spectatorTouched=false;cloud.publicUpdateAvailable=false;setMatchIndex(publicCurrentStepIndex(cloud.publicView))};$('#matchDots').innerHTML=steps.map((p,i)=>'<button type="button" class="dot '+(i===matchIndex?'active ':'')+(p.locked?'lockedDot ':'')+(i===currentStep?'currentDot':'')+'" data-match-dot="'+i+'" aria-label="'+escAttr(p.label||('Étape '+(i+1)))+'"></button>').join('');$$('[data-match-dot]').forEach(btn=>btn.onclick=()=>setMatchIndex(Number(btn.dataset.matchDot)||0));bindPublicFavoriteClicks(renderPublicMatch);$('#matchPrev').disabled=matchIndex<=0;$('#matchNext').disabled=matchIndex>=steps.length-1;let currentBtn=$('#matchCurrent');if(currentBtn)currentBtn.disabled=matchIndex===currentStep}
...
function requestStartPlay(nextHash){if(isReadOnlyMatch())return;let count=active().length;if(count<MIN||count>MAX){modal('Joueurs requis','Active entre 6 et 12 joueurs avant de commencer.');return}let ready=startReadiness(),text=ready.ok?'Le match sera débuté. Les étapes Match et Joueurs deviendront non modifiables.':'L’alignement a encore des problèmes: '+ready.text+' Tu peux continuer si c’est volontaire.';confirmModal(ready.ok?'Commencer le match':'Commencer quand même',text,()=>{beginPlay();if(nextHash)location.hash=nextHash})}
...
function route(name){let raw=name||location.hash.replace('#','')||state.route||'accueil';if(raw.startsWith('public/')){document.title='Spectateurs en direct - Rallye-Cap';openPublicRoute(raw.slice(7));return}if(raw.startsWith('fans/')){openFansRoute(raw.slice(5));return}let r=normalizeRoute(raw);if(['match','joueurs','alignement'].includes(r)&&!activeMatch()){r='accueil';raw=r;if(location.hash)history.replaceState(null,'','#'+r)}if(r==='spectateur')stopCloudMatchListener();else if(['match','joueurs','alignement'].includes(r)&&cloud.user&&cloud.matchId&&!cloud.unsubMatch)listenCloudMatch(cloud.matchId);document.title=routeTitle(r);cloud.publicView=null;cloud.publicIdRoute=null;cloud.publicTeamView=null;cloud.publicTeamRoute=null;let previous=state.route;if(r==='alignement'&&previous!==r&&!isMatchStarted())lineupMode='preparer';if(r==='spectateur'&&previous!==r)spectatorTouched=false;state.route=r;let autoOptimized=maybeAutoOptimizeLineup(r);save();if(raw!==r&&location.hash)history.replaceState(null,'','#'+r);document.body.classList.toggle('spectatorRoute',r==='spectateur');$$('.view').forEach(v=>v.classList.toggle('active',v.id==='view-'+r));$$('[data-nav],[data-step]').forEach(a=>a.classList.toggle('active',(a.dataset.nav||a.dataset.step)===r));$$('.steps').forEach(el=>el.classList.toggle('hide',r==='accueil'||r==='matchs'||r==='spectateur'));closeMainMenu();if(r!==previous)window.scrollTo({top:0,left:0,behavior:'auto'});if(autoOptimized){renderAlign();renderMini();renderOptimizeButton()}if(r==='spectateur')renderMatch();if(r==='matchs'&&cloud.user)refreshCloudMatches(true);renderCloudUi()}
...
function renderAll(){reconcileActiveMatch();analysis=null;renderInputs();renderHome();renderTeam();renderPlayers();renderAlign();renderMatch();renderMini();renderHeaderStatus();renderMatchTables();renderOptimizeButton();route()}
