...
let analysis=null, problems=new Map(), dragRow=null, dragCell=null, selectedRow=null, selectedCol=null, selectedHalf=null, highlightPos=null, statSort={key:'order',dir:1}, matchTableSort={active:{key:'date',dir:-1},archived:{key:'date',dir:-1}}, matchIndex=0, spectatorTouched=false, touchStartX=0, coachMatchIndex=0, coachTouchStartX=0, optimizeDirty=true, lineupAutoDirty=false, addPlayersExpanded=false, selectedArchiveId=null, playView='complete';
function normalizeRoute(r){return['accueil','matchs','match','joueurs','alignement','jouer'].includes(r)?r:'accueil'}
function routeTitle(r){let names={accueil:'Accueil',matchs:'Matchs',match:'Match',joueurs:'Joueurs',alignement:'Alignement',jouer:'Jouer'};return (names[r]||'Alignement')+' - Rallye-Cap'}
...
async function openBenchRoute(id){
  ...
  document.title='Banc des jeunes - Rallye-Cap';document.body.classList.remove('spectatorRoute');document.body.classList.add('benchRoute');
...
function openTeamPublicLinkModal(){
  ...
  if(!cloud.user)publicSection.append(cloudLoginButton('Connecte-toi pour créer le lien public de l’équipe.'));
    ...
    let slug=document.createElement('input');slug.id='teamPublicSlug';slug.placeholder='ex. expos-rallye-cap';slug.autocomplete='off';slug.value=team.publicSlug||'';slug.oninput=()=>{slug.value=normalizeTeamPublicIdDraft(slug.value)};
...
function scrollLineupToCurrentHalf(){let root=$('#lineup');if(!root||state.route!=='jouer'||!isMatchStarted())return;requestAnimationFrame(()=>{let th=root.querySelector('th.currentHalf');if(!th)return;root.scrollLeft=Math.max(0,th.offsetLeft-root.clientWidth*.38)})}
