...
function blankTeam(){let now=Date.now();return{id:uid(),name:'',publicId:null,publicSlug:'',publicPassword:'',roster:[],cloud:{enabled:false},createdAtMs:now,updatedAtMs:now}}
...
function privateTeamPayload(team){return{name:team?.name||'',roster:normalizePlayers(team?.roster||[]),publicId:team?.publicId||null,publicSlug:team?.publicSlug||''}}
function privateTeamPayloadJson(team){return JSON.stringify(privateTeamPayload(team))}
function applyPrivateTeamPayload(team,payload,remoteMs){if(!team||!payload)return;team.name=String(payload.name||'').trim().slice(0,80);team.roster=normalizePlayers(payload.roster);team.publicId=payload.publicId?String(payload.publicId).slice(0,100):null;team.publicSlug=normalizeTeamPublicId(payload.publicSlug||payload.publicId||'');team.cloud={enabled:true};team.updatedAtMs=Number(remoteMs)||Date.now()}
...
function benchDefense(rows,compact){let byPosition={};rows.forEach(row=>byPosition[row.position]=row);let positions=[['L1','Lanceur Gauche'],['L2','Lanceur Droite'],['AC','Arrêt-court'],['1B','Premier but'],['2B','Deuxième but'],['3B','Troisième but']];return '<div class="benchDefense '+(compact?'compact':'')+'">'+positions.map(([code,label])=>{let row=byPosition[code]||{name:'—'},icons=(code==='L1'||code==='L2'?'🧢 ':'')+'🧤';return '<div class="benchDefender"><span class="benchDefenseRole"><strong>'+icons+' '+esc(label)+'</strong><small>'+esc(code)+'</small></span><span>'+benchPlayerName(row)+'</span></div>'}).join('')+'</div>'}
...
function openTeamPublicLinkModal(){
  ...
  let publicText=document.createElement('p');publicText.className='hint';publicText.textContent='Un lien permanent à partager avec les parents et les fans.';publicSection.appendChild(publicText);
  if(!cloud.user)publicSection.append(cloudLoginButton('Connecte-toi pour créer le lien public de l’équipe.'));
    ...
    let slug=document.createElement('input');slug.id='teamPublicSlug';slug.placeholder='ex. expos-rallye-cap';slug.autocomplete='off';slug.value=team.publicSlug||'';slug.oninput=()=>{slug.value=normalizeTeamPublicId(slug.value)};
    let slugLabel=document.createElement('label');slugLabel.innerHTML='<span>Identifiant public</span>';slugLabel.appendChild(slug);publicSection.appendChild(slugLabel);
  ...
  let manage=modalSection('Gérer en ligne'),manageText=document.createElement('p');manageText.className='hint';manageText.textContent='Synchronise le nom et le bassin privé de joueurs entre tes appareils.';manage.appendChild(manageText);
